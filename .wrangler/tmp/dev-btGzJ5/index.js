...
var getSpaInfo = /* @__PURE__ */ __name(async (c) => {
  try {
    ...
    const { data, error: error3 } = await sb.from("SpaInfo").select("*").single();
...
var getStats = /* @__PURE__ */ __name(async (c) => {
  try {
    const sb = getSupabase(c.env);
    const { data: appointments, error: error3 } = await sb.from("Appointment").select("status, startTime, clientTier, price");
    if (error3) return c.json({ error: error3.message }, 500);
    ...
    const totalRevenueVND = allAppts.filter(
      (a) => a.status === "completed" || a.status === "in_progress" || a.status === "confirmed"
    ...
    return c.json({
      ...
      revenueToday: (totalRevenueVND / 1e6).toFixed(1) + "M",
      revenueTarget: 85,
...
var getClients = /* @__PURE__ */ __name(async (c) => {
  try {
    const sb = getSupabase(c.env);
    const { data, error: error3 } = await sb.from("Client").select("*").order("name", { ascending: true });
    if (error3) return c.json({ error: error3.message }, 500);
    return c.json(serialize(data));
  } catch (error3) {
    return c.json({ error: error3.message }, 500);
...
var createClient2 = /* @__PURE__ */ __name(async (c) => {
  try {
    const sb = getSupabase(c.env);
    const body = await c.req.json();
    ...
    const { data, error: error3 } = await sb.from("Client").insert({
      id: crypto.randomUUID(),
      name,
      avatar: avatar || "",
      tier: tier || "Th\xE0nh Vi\xEAn",
      phone,
      email: email || "",
      totalVisits: 0,
      totalSpent: 0,
...
var updateClient = /* @__PURE__ */ __name(async (c) => {
  try {
    const sb = getSupabase(c.env);
    ...
    if (tier !== void 0) updateData.tier = tier;
    if (phone !== void 0) updateData.phone = phone;
    if (email !== void 0) updateData.email = email;
    if (notes !== void 0) updateData.notes = notes;
    if (totalVisits !== void 0) updateData.totalVisits = totalVisits;
    if (totalSpent !== void 0) updateData.totalSpent = totalSpent;
    if (memberPoints !== void 0) updateData.memberPoints = memberPoints;
    if (lastVisit !== void 0) updateData.lastVisit = lastVisit;
    if (joinDate !== void 0) updateData.joinDate = joinDate;
    const { data, error: error3 } = await sb.from("Client").update(updateData).eq("id", id).select().single();
    if (error3) return c.json({ error: error3.message }, 500);
    return c.json(serialize(data));
  } catch (error3) {
    return c.json({ error: error3.message }, 500);
...
var deleteClient = /* @__PURE__ */ __name(async (c) => {
  try {
    const sb = getSupabase(c.env);
    const id = c.req.param("id");
    const { error: error3 } = await sb.from("Client").delete().eq("id", id);
    if (error3) return c.json({ error: error3.message }, 500);
    return c.json({ success: true, message: "\u0110\xE3 x\xF3a kh\xE1ch h\xE0ng" });
  } catch (error3) {
    return c.json({ error: error3.message }, 500);
  }
}, "deleteClient");
...
var getServices = /* @__PURE__ */ __name(async (c) => {
  try {
    const sb = getSupabase(c.env);
    const { data, error: error3 } = await sb.from("Service").select("*").order("name", { ascending: true });
    if (error3) return c.json({ error: error3.message }, 500);
    return c.json(serialize(data));
  } catch (error3) {
    return c.json({ error: error3.message }, 500);
...
var getTherapists = /* @__PURE__ */ __name(async (c) => {
  try {
    const sb = getSupabase(c.env);
    const { data, error: error3 } = await sb.from("Therapist").select("*").order("name", { ascending: true });
    if (error3) return c.json({ error: error3.message }, 500);
    return c.json(serialize(data));
  } catch (error3) {
    return c.json({ error: error3.message }, 500);
...
var createTherapist = /* @__PURE__ */ __name(async (c) => {
  try {
    ...
    if (!name || !specialty) {
      return c.json({ error: "T\xEAn v\xE0 chuy\xEAn m\xF4n l\xE0 b\u1EAFt bu\u1ED9c" }, 400);
    }
    const id = crypto.randomUUID();
    const { data, error: error3 } = await sb.from("Therapist").insert({
      id,
      name,
      avatar: avatar || "",
      specialty,
...
var updateTherapist = /* @__PURE__ */ __name(async (c) => {
  try {
    ...
    if (services !== void 0) updateData.services = services;
    if (availability !== void 0) updateData.availability = availability;
    const { data, error: error3 } = await sb.from("Therapist").update(updateData).eq("id", id).select().single();
    if (error3) return c.json({ error: error3.message }, 500);
    return c.json(serialize(data));
  } catch (error3) {
    return c.json({ error: error3.message }, 500);
...
var deleteTherapist = /* @__PURE__ */ __name(async (c) => {
  try {
    const sb = getSupabase(c.env);
    const id = c.req.param("id");
    const { error: error3 } = await sb.from("Therapist").delete().eq("id", id);
    if (error3) return c.json({ error: error3.message }, 500);
    return c.json({ success: true, message: "\u0110\xE3 x\xF3a nh\xE2n vi\xEAn" });
  } catch (error3) {
    return c.json({ error: error3.message }, 500);
  }
}, "deleteTherapist");
...
var getAppointments = /* @__PURE__ */ __name(async (c) => {
  try {
    const sb = getSupabase(c.env);
    const { data, error: error3 } = await sb.from("Appointment").select("*").order("date", { ascending: false }).order("startTime", { ascending: true });
    if (error3) return c.json({ error: error3.message }, 500);
    return c.json(serialize(data));
  } catch (error3) {
    return c.json({ error: error3.message }, 500);
...
var createAppointment = /* @__PURE__ */ __name(async (c) => {
  try {
    ...
    const [clientRes, therapistRes, existRes] = await Promise.all([
      ...
      sb.from("Appointment").select("id, startTime, endTime, status, therapist").eq("therapistId", therapistId).eq("date", date).neq("status", "cancelled")
    ]);
    ...
    if (existRes.data && existRes.data.length > 0) {
      const parseTimeToMinutes = /* @__PURE__ */ __name((t) => {
        if (!t) return 0;
      ...
      if (conflict) {
        return c.json(
          {
            error: `Tr\xF9ng l\u1ECBch h\u1EB9n! Nh\xE2n vi\xEAn ${therapistRes.data.name} \u0111\xE3 c\xF3 l\u1ECBch t\u1EEB ${conflict.startTime} \u0111\u1EBFn ${conflict.endTime} trong ng\xE0y n\xE0y.`
          },
          400
        );
      }
...
var updateAppointment = /* @__PURE__ */ __name(async (c) => {
  try {
    ...
    if (userRole === "technician") {
      const hasChanges = service !== void 0 && service !== currentApt.service || price !== void 0 && Number(price || 0) !== currentApt.price || date !== void 0 && date !== currentApt.date || startTime !== void 0 && startTime !== currentApt.startTime || endTime !== void 0 && endTime !== currentApt.endTime || therapistId !== void 0 && therapistId !== currentApt.therapistId || status !== void 0 && status !== currentApt.status;
      if (hasChanges) {
        return c.json(
          {
            error: "Quy\u1EC1n truy c\u1EADp b\u1ECB t\u1EEB ch\u1ED1i. K\u1EF9 thu\u1EADt vi\xEAn ch\u1EC9 \u0111\u01B0\u1EE3c ph\xE9p c\u1EADp nh\u1EADt ghi ch\xFA tr\u1ECB li\u1EC7u."
          },
          403
        );
      }
    }
    const finalDate = date !== void 0 ? date : currentApt.date;
    const finalTherapistId = therapistId !== void 0 ? therapistId : currentApt.therapistId;
    const finalStartTime = startTime !== void 0 ? startTime : currentApt.startTime;
    const finalEndTime = endTime !== void 0 ? endTime : currentApt.endTime;
    const finalStatus = status !== void 0 ? status : currentApt.status;
    if (finalStatus !== "cancelled") {
      const { data: existAppts, error: checkError } = await sb.from("Appointment").select("id, startTime, endTime, status, therapist").eq("therapistId", finalTherapistId).eq("date", finalDate).neq("status", "cancelled").neq("id", id);
      if (checkError) {
      ...
      if (existAppts && existAppts.length > 0) {
        ...
        if (conflict) {
          return c.json(
            {
              error: `Tr\xF9ng l\u1ECBch h\u1EB9n! Nh\xE2n vi\xEAn ${conflict.therapist || "KTV"} \u0111\xE3 c\xF3 l\u1ECBch t\u1EEB ${conflict.startTime} \u0111\u1EBFn ${conflict.endTime} trong ng\xE0y n\xE0y.`
            },
            400
          );
        }
    ...
    if (price !== void 0) updateData.price = Number(price || 0);
    ...
    if (therapistId) {
      const { data: th, error: thErr } = await sb.from("Therapist").select("id, name").eq("id", therapistId).single();
      if (thErr || !th) return c.json({ error: "Nh\xE2n vi\xEAn kh\xF4ng t\u1ED3n t\u1EA1i" }, 404);
      updateData.therapistId = therapistId;
      updateData.therapist = th.name;
    }
    const { data, error: error3 } = await sb.from("Appointment").update(updateData).eq("id", id).select().single();
    if (error3) return c.json({ error: error3.message }, 500);
    return c.json(serialize(data));
  } catch (error3) {
    return c.json({ error: error3.message }, 500);
...
var updateAppointmentStatus = /* @__PURE__ */ __name(async (c) => {
  try {
    ...
    if (!status) return c.json({ error: "Tr\u1EA1ng th\xE1i l\xE0 b\u1EAFt bu\u1ED9c" }, 400);
    const labels = {
      in_progress: "\u0110ang x\u1EED l\xFD",
      completed: "Ho\xE0n t\u1EA5t",
      cancelled: "\u0110\xE3 h\u1EE7y",
    ...
    const { data, error: error3 } = await sb.from("Appointment").update({ status, statusLabel: labels[status] || "\u0110\xE3 x\xE1c nh\u1EADn" }).eq("id", id).select().single();
    if (error3) return c.json({ error: error3.message }, 500);
    return c.json(serialize(data));
  } catch (error3) {
    return c.json({ error: error3.message }, 500);
...
var deleteAppointment = /* @__PURE__ */ __name(async (c) => {
  try {
    const sb = getSupabase(c.env);
    const id = c.req.param("id");
    const { error: error3 } = await sb.from("Appointment").delete().eq("id", id);
    if (error3) return c.json({ error: error3.message }, 500);
...
var getWorkflows = /* @__PURE__ */ __name(async (c) => {
  try {
    const sb = getSupabase(c.env);
    const { data, error: error3 } = await sb.from("Workflow").select("*");
    if (error3) return c.json({ error: error3.message }, 500);
    return c.json(serialize(data));
  } catch (error3) {
    return c.json({ error: error3.message }, 500);
...
var toggleWorkflow = /* @__PURE__ */ __name(async (c) => {
  try {
    const sb = getSupabase(c.env);
    const id = c.req.param("id");
    const { data: workflow, error: fetchError } = await sb.from("Workflow").select("id, active").eq("id", id).single();
    if (fetchError || !workflow) {
      return c.json({ error: "Workflow kh\xF4ng t\u1ED3n t\u1EA1i" }, 404);
    }
    const { data, error: error3 } = await sb.from("Workflow").update({ active: !workflow.active }).eq("id", id).select().single();
    if (error3) return c.json({ error: error3.message }, 500);
    return c.json(serialize(data));
  } catch (error3) {
    return c.json({ error: error3.message }, 500);
...
var getNotifications = /* @__PURE__ */ __name(async (c) => {
  try {
    const sb = getSupabase(c.env);
    const { data, error: error3 } = await sb.from("Notification").select("*").order("id", { ascending: false });
    if (error3) return c.json({ error: error3.message }, 500);
    return c.json(serialize(data));
  } catch (error3) {
    return c.json({ error: error3.message }, 500);
...
var markAllNotificationsAsRead = /* @__PURE__ */ __name(async (c) => {
  try {
    const sb = getSupabase(c.env);
    const { error: error3 } = await sb.from("Notification").update({ read: true }).eq("read", false);
    if (error3) return c.json({ error: error3.message }, 500);
...
var deleteNotification = /* @__PURE__ */ __name(async (c) => {
  try {
    const sb = getSupabase(c.env);
    const id = c.req.param("id");
    const { error: error3 } = await sb.from("Notification").delete().eq("id", id);
    if (error3) return c.json({ error: error3.message }, 500);
...
var updateCurrentUser = /* @__PURE__ */ __name(async (c) => {
  try {
    const sb = getSupabase(c.env);
    const { role } = await c.req.json();
    if (!role || !["admin", "cashier", "technician"].includes(role)) {
      return c.json({ error: "Vai tr\xF2 kh\xF4ng h\u1EE3p l\u1EC7" }, 400);
    }
    const { data: user, error: fetchErr } = await sb.from("CurrentUser").select("*").single();
    if (fetchErr || !user) {
      return c.json({ error: "Kh\xF4ng t\xECm th\u1EA5y ng\u01B0\u1EDDi d\xF9ng hi\u1EC7n t\u1EA1i" }, 404);
    }
    let updateData = {
      role,
      name: "Nguy\u1EC5n Minh Admin",
      avatar: "https://images.unsplash.com/photo-1534528741775-53994a69daeb?auto=format&fit=crop&w=150&h=150&q=80",
      greeting: "Ch\xE0o m\u1EEBng Qu\u1EA3n tr\u1ECB vi\xEAn"
    };
    if (role === "cashier") {
      updateData = {
        role,
        name: "Tr\u1EA7n Thanh Thu Ng\xE2n",
        avatar: "https://images.unsplash.com/photo-1573496359142-b8d87734a5a2?auto=format&fit=crop&w=150&h=150&q=80",
        greeting: "Ch\xE0o m\u1EEBng Thu ng\xE2n"
      };
    } else if (role === "technician") {
      updateData = {
        role,
        name: "L\xEA An K\u1EF9 Thu\u1EADt Vi\xEAn",
        avatar: "https://images.unsplash.com/photo-1580489944761-15a19d654956?auto=format&fit=crop&w=150&h=150&q=80",
        greeting: "Ch\xE0o K\u1EF9 thu\u1EADt vi\xEAn"
      };
    }
    const { data, error: error3 } = await sb.from("CurrentUser").update(updateData).eq("id", user.id).select().single();
    if (error3) return c.json({ error: error3.message }, 500);
    return c.json(serialize(data));
  } catch (error3) {
    return c.json({ error: error3.message }, 500);
...
${query}`;
  ...
  try {
    const res = await fetch(url, {
      ...
      body: JSON.stringify({ name: event, channels: [channel2], data: body })
...
var checkAdmin = /* @__PURE__ */ __name(async (c, next) => {
  ...
  if (user?.role !== "admin") {
    return c.json(
      {
        error: "Quy\u1EC1n truy c\u1EADp b\u1ECB t\u1EEB ch\u1ED1i. Ch\u1EC9 Qu\u1EA3n tr\u1ECB vi\xEAn m\u1EDBi th\u1EF1c hi\u1EC7n \u0111\u01B0\u1EE3c h\xE0nh \u0111\u1ED9ng n\xE0y."
      },
      403
    );
  }
...
var checkNotTechnician = /* @__PURE__ */ __name(async (c, next) => {
  const sb = getSupabase(c.env);
  const { data: user } = await sb.from("CurrentUser").select("role").single();
  if (user?.role === "technician") {
    return c.json(
      {
        error: "Quy\u1EC1n truy c\u1EADp b\u1ECB t\u1EEB ch\u1ED1i. K\u1EF9 thu\u1EADt vi\xEAn kh\xF4ng \u0111\u01B0\u1EE3c th\u1EF1c hi\u1EC7n h\xE0nh \u0111\u1ED9ng n\xE0y."
      },
      403
    );
  }
...
router.post("/clients", createClient2);
...
// .wrangler/tmp/bundle-u2qqpr/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-u2qqpr/middleware-loader.entry.ts
